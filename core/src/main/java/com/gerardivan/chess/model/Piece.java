...
public class Piece {
    ...
    /**
     * Booleà per comprovar si la peça pot fer un moviment, ens envia al mètode concret
    ...
    */
    public boolean canMoveTo(Board board, int destX, int destY) {
        ...
        if (x == destX && y == destY) return false; //si la posició destí i origen és la mateixa
    ...
    private boolean canMovePeon(Board board, int x, int y, int dx, int dy) {
        ...
        if (colDiff == 0) {
            ...
            if (rowDiff == 2 * dir && target == null && ultimaPosicio.equals(posicio))
                //afegim condició, si no hi ha cap peça a la casella de just davant, retornarà true
                return board.getPiece(x, y + dir) == null;
        }

        // Captura diagonal
        if (Math.abs(colDiff) == 1 && rowDiff == dir) {
            return target != null && target.esBlanca != esBlanca;
        }
    ...
    private boolean canMoveCavall(int x, int y, int dx, int dy) {
    ...
    private boolean canMoveReina(Board board, int x, int y, int dx, int dy) {
        ...
        if (!recte && !diagonal) return false;
        return board.isPathClear(x, y, dx, dy);
    }

    private boolean canMoveRei(int x, int y, int dx, int dy) {
